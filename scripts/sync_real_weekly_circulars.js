...
async function scrapeIcaOffers(store) {
  try {
    ...
    return rawOffers.map((item) => {
      ...
      return {
        ...
        image_url: imageUrl,
        compare_price: item.comparePrice || (regPrice > 0 ? (regPrice * 1.2).toFixed(2) + ' kr/kg' : undefined),
...
async function scrapeAxfoodOffers(chain, storeId) {
...
function mapCategoryName(catStr) {
  ...
  if (c.includes('dryck')) return 'Dryck & Godis';
  if (c.includes('godis') || c.includes('snacks')) return 'Dryck & Godis';
  return 'Skafferi';
}
...
async function syncAllRealWeeklyCirculars() {
  ...
  for (const st of ICA_STORES) {
    ...
    if (liveOffers.length > 0) {
      ...
      for (let i = 0; i < liveOffers.length; i += 50) {
        const chunk = liveOffers.slice(i, i + 50);
        const { error } = await supabase.from('offers').insert(chunk);
        if (error) console.error(`  Kunde inte spara i Supabase for ${st.name}:`, error.message);
      }
    }
  }
  ...
  if (willysOffers.length > 0) {
    ...
    for (let i = 0; i < willysOffers.length; i += 50) {
      const chunk = willysOffers.slice(i, i + 50);
      await supabase.from('offers').insert(chunk);
    }
  }
  ...
  if (hemkopOffers.length > 0) {
    ...
    for (let i = 0; i < hemkopOffers.length; i += 50) {
      const chunk = hemkopOffers.slice(i, i + 50);
      await supabase.from('offers').insert(chunk);
    }
  }
